...
from corpus_python import Corpus
    ...
    def fit(self, matrix, epochs=5, no_threads=2, verbose=False, wordList = None,save_gap=None, retrain = False, save_vectors = False):
        ...
        for epoch in range(epochs):
            ...
            if epoch%save_gap==0 and wordList is not None:
                ...
                # self.saveState(df,epoch)
                
    def loadStartIndex(self,index):
        self.startIndex= index

    def saveFig(self, df, epoch):
        df.to_csv("output_"+str(epoch)+".csv")
        # plt.figure(figsize = (10,10))
        # plot = sns.heatmap(df)
        # fig = plot.get_figure()
        # fig.savefig("output_"+str(epoch)+".png")
        # fig.clf()
